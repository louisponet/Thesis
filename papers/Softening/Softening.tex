...
The uniform mesh is largely motivated by implementation complexity, since we use a mostly in-house developed code based on the FEM building blocks supplied by the JuAFEM.jl package \lp{cite + link?}.
...
This requires us to define the partial derivatives of the total energy in terms of all the DOFs, for which we use a forward automatic differentiation scheme as implemented in ForwardDiff.jl, again for its remarkable simplicity and speed (as an idea, on a 64 core Epyc cluster, the $\approx$ 4 million partial derivatives are computed in a couple of seconds).
