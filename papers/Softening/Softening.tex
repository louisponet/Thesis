...
In the extreme case, certain ferroelectric DWs can even become conductive, opening the door to very promising technological applications \cite{Seidel2009}.
...
\\\\
While this kind of technological promise has historically driven most of the research of DWs towards their electronic properties, interest for their mechanical characteristics has increased in more recent years.
...
Afterwards, we continue with a short overview of the crystalline properties of BaTiO$_3$ in Section~\ref{sec:BTO_crystal}, an archetypal example of a ferroelectric perovskite.
This is followed by a detailed theoretical study in Section~\ref{sec:BTO_theory}, a description of our numerical simulations in Section~\ref{sec:BTO_methods}, and a discussion of the obtained results in Section~\ref{sec:BTO_results}.
Finally we conclude by summarizing the results in Section~\ref{sec:BTO_conclusion}.
...
These phases are exactly what is shown in panels (a,c) of Fig.~\ref{fig:BTO_experiment}. The amplitude of the oscillation gives an idea on the absolute value of the polarization, which leads to the darker regions of the DWs in panel (d) of the same figure.
...
However, as we will touch on in further detail when the simulation technique is discussed in Section~\ref{sec:BTO_methods}, a numerical analogue to these inter-cell barriers appears~\cite{Marton2018}.
...
\begin{figure}
	...
	\caption{\label{fig:BTO_breathing_sliding}{\bf Breathing and Sliding modes.} a) Demonstration of the widening of the DW upon application of a tip, representing a breathing mode. The higher the force of the tip, the wider the wall becomes. The wall is not centered at 0 because of the numerical Peierls-Nabarro and mesh dimensions, however, the applied force is centered exactly above the wall. b) Pictorial demonstration of the sliding mode, which get excited by the tip, making the wall slide towards it.}
\end{figure}
\\\\
...
The movement of the preexisting dip towards the tip then leads to a relatively large deformation, making the material appear soft.
...
This in turn results in the numerical analogue of the Peierls-Nabarro barrier.
...
The former behaves very similar to the latter, except that it acquires an additional penalty due to the increased DW area and non-zero Peierls-Nabarro barriers that the DW is located on.
