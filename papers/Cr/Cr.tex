...
By using ultrafast pump-probe experiments that have a sub picosecond resolution, these PLD dynamics can be studied and, more imporantly, controlled very precisely through excitation of the much faster responding SDW.
...
The double well potential that causes the SDW phase transition is characterised by the coefficients $\alpha$ and $\beta$,  and critical temperature $T_N$ below which the SDW order sets in.
...
For example, it was known that the opticial pulse width $\tau$ was around 40 fs, the oscillation frequency of the PLD $\omega_0 \approx$ 14 THz or equivalently had a period of around 450 fs.
...
This can be understood through a thought experiment of a hypotethical system for which the the PLD is the primary order parameter, with associated double well potential similar, and no other coupled order parameters exist.
The only possibility to excite an oscillation in this case is by heating up the system above the critical temperature.
This would change the shape of the hypotethical energy potential, shifting its minimum to 0, and again due to the slow dynamics an oscillation would occur.
