...
\newcommand{\WanKet}[2]{\Ket{\Wan{#1}{#2}(\bm{r})}}
\newcommand{\tildeWanKet}[2]{\Ket{\tildeWan{#1}{#2}(\bm{r})}}
\newcommand{\ShortWanKet}[2]{\Ket{\Wan{#1}{#2}}}
\newcommand{\tildeShortWanKet}[2]{\Ket{\tildeWan{#1}{#2}}}
\newcommand{\WanBra}[2]{\Bra{\Wan{#1}{#2}(\bm{r})}}
\newcommand{\tildeWanBra}[2]{\Bra{\tildeWan{#1}{#2}(\bm{r})}}
\newcommand{\ShortWanBra}[2]{\Bra{\Wan{#1}{#2}}}
\newcommand{\tildeShortWanBra}[2]{\Bra{\tildeWan{#1}{#2}}}
...
\begin{equation}
	...
	\BlochKet{n} = \eikr{r} \unkrKet{n}{k} = \sum_{\bm{R}} \eikr{R} \WanKet{n}{R},
\end{equation}
...
\begin{equation}
	\Omega =\sum_n \ShortWanBra{n}{0}r^2\ShortWanKet{n}{0} - \left|\ShortWanBra{n}{0}\bm{r}\ShortWanKet{n}{0}\right|^2.
\end{equation}
...
\begin{equation}
\ShortWanBra{n}{R}\bm{r}\ShortWanKet{m}{0} = i \frac{V}{(2\pi)^3}\int d\bm{k} \eikr{R}\unkBra{n}{k} \bm{\nabla}_{\bm{k}} \unkKet{m}{k}
\end{equation}
and
\begin{equation}
\ShortWanBra{n}{R}r^2\ShortWanKet{m}{0} = -\frac{V}{(2\pi)^3}\int d\bm{k} \eikr{R}\unkBra{n}{k} \bm{\nabla}_{\bm{k}}^2 \unkKet{m}{k}.
\end{equation}
...
\begin{equation}
	\tilde{H}_{nm}^{\bm{k}} = \tildeBlochBra{n} H \tildeBlochKet{m} = \sum_{\bm{R}} \eikr{R} \ShortWanBra{n}{0}H\ShortWanKet{n}{R},
\end{equation}
where the tilde is used to distinguish the $k$-space wavefunctions from BFs, and $\ShortWanKet{n}{R}$ denote the WFs centered at unit cell identified by lattice translation $\bm{R}$.
