...
The breaking of inversion symmetry in these materials leads to an internal electric field.
However, contrary to the tiny effect expected from the above arguments, a sizeable linear energy splitting between spin-polarized states can appear in these materials \cite{DiSante2013}.
This splitting manifests itself in the bandstructure as conical intersections surrounding time-reversal symmetric points of the Brillouin Zone (BZ) (see Fig.~\ref{fig:Rashba_intro_dispersion}).
These points are special in the sense that by applying the time-reversal operator to the wavevector ($\bm k \rightarrow \bm {-k}$), they are brought to opposing points of the BZ that are separated by a reciprocal lattice translation, and are thus equivalent.\lp{if time make a figure}
Since time-reversal reverses the spin, this means that both up and down spin states are equivalent at these points, leading to the intersection demonstrated in Fig.~\ref{fig:Rashba_intro_dispersion}.
...
Since the sign of the electric field that leads to the spin-splitting is a result of the orientation of the ferroelectric polarization, it can be reversed by the application of a sufficiently strong external electric field (to reverse the polarization of the material).
