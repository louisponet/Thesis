...
To do this, we separate $\bm E = \bm{\nabla}V$ into two contributions, the first coming from the potential wells around the atoms ($E_{\rm at}$), and the second originating from the internal field due to the polarization ($E_{\rm P}$)\footnote{We assume here that no external fields are applied, these can be included trivially by summing them to the internal field.}:
\begin{equation}
	\bm{\nabla} V = \bm{E} = \bm{E}_{\rm at} + \bm{E}_{\rm P}
\end{equation}
