...
Using Eq.~\ref{eq:Rashba_blochfromwan} allows us to calculate the OAM of the Bloch functions as
\begin{equation}
...
Moreover, $k_x$ remains zero along the $A$--$Z$, and we can observe that only $L_x$ develops due to the nonzero $k_y$. Only when $k_x$ is nonzero, along the $Z$--$U$ path, does $L_y$ show up. This is fully consistent with the expressions for $L_x$ and $L_y$ in Eqs.~\eqref{eq:oam_from_k}--\eqref{eq:oam_from_k_final}.
...
Combining these terms led us to the newly proposed Rashba-like effective Hamiltonian in Eq.~\ref{eq:Rashba_hami}.
\\\\
...
And finally: to maximize the unquenching of the OAM, a highly isotropic environment that minimizes the electrostatic penalty of having nonzero OAM is beneficial.
